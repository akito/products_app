...
RSpec.describe Product, type: :model do
  ...
  describe "scope" do
    describe "created_after(arg = 1.weeks.ago )" do
      let!(:old_product) { create(:product, created_at: 1.week.ago) }
      ...
      context "when product created one week ago" do
        it "dose NOT include the product" do
          expect(Product.created_after(1.week.ago)).not_to include old_product
        end
      end
      context "when product created yesterday" do
        it "includes the product " do
          expect(Product.created_after(1.week.ago)).to include new_product
        end
      end
    end
